...
# bash <(curl -sL "https://onedrive.live.com/download?cid=B3B1906452EBEA7C&resid=b3b1906452ebea7c%2115544&authkey=AIF_x8RzWDO8iYQ")
...
getData() {
    if [[ "$TLS" = "true" || "$XTLS" = "true" ]]; then
        echo ""
        colorEcho ${YELLOW} "预先前提：伪装域名（${DOMAIN}）DNS解析指向当前服务器ip（${IP}）"
        echo ""
...
menu() {
    clear
    echo "#############################################################"
    echo -e "#                ${RED}trojan+xtls 一键安装脚本${PLAIN}                   #"
    echo "#############################################################"
